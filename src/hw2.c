...
{
    unsigned int* completions = malloc(1024 * sizeof(unsigned int));
    
    int index = 0;
    for (unsigned int i = 0; ; ) {

        unsigned int int0 = packets[0];
        unsigned int int1 = packets[1];
        unsigned int int2 = packets[2];
        ...
        completions[index++] = (0x094000 << 10) | length;
        completions[index++] = (((0x00DC << 4) | 0x0) << 10) | length * 4;
        completions[index++] = ((((requester_id << 8) | tag) << 8) | lower_address);

        for (unsigned int j = 0; j < beforeBound; j++) {
            completions[index++] = (((((memory[address + j * 4 + 3] << 8) | memory[address + j * 4 + 2]) << 8) | memory[address + j * 4 + 1]) << 8) | memory[address + j * 4];

        }
        
        if (beforeBound != length) {
            completions[index++] = (0x094000 << 10) | (length - beforeBound);
            completions[index++] = (((0x00DC << 4) | 0x0) << 10) | (length - beforeBound) * 4;
            completions[index++] = ((((requester_id << 8) | tag) << 8) | (0x4000 & 0x7F));

            for (unsigned int j = 0; j < length - beforeBound; j++) {
            completions[index++] = (((((memory[address + j * 4 + 3] << 8) | memory[address + j * 4 + 2]) << 8) | memory[address + j * 4 + 1]) << 8) | memory[address + j * 4];
